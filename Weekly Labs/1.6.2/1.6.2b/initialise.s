...
milliseconds_delay:
	LDR R0, =TIM2
	LDR R1, =799                     @ set the PSC=799 (100us=0.1ms)
	STR R1, [R0, TIM_PSC]
	LDR R6,=10000                    @1s ON/OFF

	BX LR

microsecond_delay:
    LDR R0, =TIM2
	LDR R1, =7                     @ Set the PSC=7 (1us)
	STR R1, [R0, TIM_PSC]
	LDR R6,=1000000                @1s ON/OFF

	BX LR

second_delay:
    LDR R0, =TIM2
	LDR R1, =79999                    @ Set the PSC= 79999 (100ms)
	STR R1, [R0, TIM_PSC]

	LDR R6,=1000                     @1s ON/OFF
