...
main:
    ...
    MOV R5, #0x0D  			  @ Terminating character

read_string:
    LDR R0, =UART  	          @ Base address for UART1
    LDR R6, [R0, #USART_ISR]  @ Load UART status

    TST R6, #(1 << UART_ORE | 1 << UART_FE)  @ Check for overrun or framing error
    ...
    BEQ end_read  			   @ If it matches, exit loop
...
transmit_loop:
    ...
    TST R1, #(1 << UART_TXE)  	@ Check if transmit buffer is empty
